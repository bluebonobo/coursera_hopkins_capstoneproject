...
This document explains the major features of the data and summarize how I plan to create a prediction algorithm and Shiny app using the knowledge acquired during the specialization.


# Exploratory Analysis

For performance and convenience of this week's presentation, I have extracted a random subset of the files provided (blogs, news and twitter files) for the capstone project. We will also focus on the English language files. In order to address the full data set, we will need to optimize performance and libraries used. I have only used the tm library so far but I am planning to switch in the coming weeks. 
...
For each file, we build a VCorpus object, we clean the VCorpus by removing whitepaces, convert all to lower case, remove English stop words, remove punctuation, remove numbers, remove profanity and stem. We then tokenzie and finally create a DocumentTermMatrix.
...
The Sample Blogs file we are going to analyze consists of **`r lapply(working_english_corpus_blogs[[1]][1],length)` lines** and **`r dim(dtm_blogs)[2]` terms**. Review of the document term matrix here `r inspect(dtm_blogs)`
...
The Sample Blogs file we are going to analyze consists of **`r lapply(working_english_corpus_news[[1]][1],length)` lines** and **`r dim(dtm_news)[2]` terms**. Review of the document term matrix here `r inspect(dtm_news)`
...
The Sample Blogs file we are going to analyze consists of **`r lapply(working_english_corpus_twitter[[1]][1],length)` lines** and **`r dim(dtm_twitter)[2]` terms**. Review of the document term matrix here `r inspect(dtm_twitter)`
...
For the prediction part of the assignment, I intend to build an ngrams model for 3,4,5 words. The algorithm will predict the next word based on the first 3,4 words provided in the App. 
