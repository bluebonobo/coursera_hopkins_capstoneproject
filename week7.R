...
# trigramFreqTerms<- findFreqTerms(trigramDocumentMatrix, lowfreq=5)
...
start_time_fourgrams <- Sys.time()

Fourgramtokenizer <- function(x) unlist(lapply(ngrams(words(x), 4), paste, collapse = " "), use.names = FALSE)
fourgramDocumentMatrix <- TermDocumentMatrix(working_english_corpus_blogs,control = list(tokenize = Fourgramtokenizer))
fourgramFreqTerms<- findFreqTerms(fourgramDocumentMatrix, lowfreq=5)
fourgramFreq <- rowSums(as.matrix(fourgramDocumentMatrix[fourgramFreqTerms,]))
fourgramFreq <- data.frame(word=names(fourgramFreq),frequency=fourgramFreq)
fourgramFreq_sort<- fourgramFreq[order(fourgramFreq$frequency),]

fourgramTable <- separate(fourgramFreq_sort, col=word, into=c('word1', 'word2', 'word3', 'word4'), sep = " ")

print(dim(fourgramTable))
print(tail(fourgramTable))

end_time_fourgrams <- Sys.time()
print(paste("fourgrams execution time >> ", end_time_fourgrams - start_time_fourgrams))

saveRDS(fourgramTable, "./ngramsTables/fourgramTable.rds")

print ("========================= 5Grams =========================")

start_time_fivegrams <- Sys.time()

# Tokenize Corpus Bigrams
Fivegramtokenizer <- function(x) unlist(lapply(ngrams(words(x), 4), paste, collapse = " "), use.names = FALSE)
fivegramDocumentMatrix <- TermDocumentMatrix(working_english_corpus_blogs,control = list(tokenize = Fivegramtokenizer))
fivegramFreqTerms<- findFreqTerms(fivegramDocumentMatrix, lowfreq=0)
fivegramFreq <- rowSums(as.matrix(fivegramDocumentMatrix[fivegramFreqTerms,]))
fivegramFreq <- data.frame(word=names(fivegramFreq),frequency=fivegramFreq)
fivegramFreq_sort<- fivegramFreq[order(fivegramFreq$frequency),]

fivegramTable <- separate(fivegramFreq_sort, col=word, into=c('word1', 'word2', 'word3', 'word4', 'word5'), sep = " ")

print(dim(fivegramTable))
print(tail(fivegramTable))

end_time_fivegrams <- Sys.time()
print(paste("fivegrams execution time >> ", end_time_fivegrams - start_time_fivegrams))

saveRDS(fivegramTable, "./ngramsTables/fivegramTable.rds")
